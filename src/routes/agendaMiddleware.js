...
// Capa mínima y aislada para el calendario del agendamiento.
// No reemplaza el webhook principal: solo toma los pasos de selección de fecha
// y deja que src/routes/whatsapp.js siga manejando todo lo demás.
...
function claveContenido(from, text) {
  return `agenda:${normalizarTelefono(from)}::${String(text || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")}`;
}
...
function esOpcionRenovacion(msg) {
  ...
  return (
    ...
    texto === "renovar" ||
    texto === "refrendar" ||
    texto.includes("renovar licencia") ||
    texto.includes("renovación de licencia") ||
    texto.includes("renovacion de licencia")
  );
}
...
function esOpcionPrimeraVez(msg) {
  ...
  return (
    texto === "2" ||
    texto === "primera vez" ||
    texto.includes("por primera vez") ||
    texto.includes("primera licencia") ||
    texto.includes("sacar licencia")
  );
}

function tramiteDirecto(session, text) {
  const msg = String(text || "").toLowerCase().trim();

  if (session.step === "MENU_TRAMITE") {
    if (esOpcionRenovacion(msg)) return "Renovación / Refrendación";
    if (esOpcionPrimeraVez(msg)) return "Primera vez";
    return null;
  }
  ...
  if (frasesRenovacion.some((frase) => msg.includes(frase))) {
    return "Renovación / Refrendación";
  }

  const frasesPrimeraVez = [
  ...
  if (frasesPrimeraVez.some((frase) => msg.includes(frase))) {
    return "Primera vez";
  }
...
function esComandoFlujoNormal(text) {
  const msg = String(text || "").toLowerCase().trim();
  return (
...
async function responderPreguntasRapidas(from, text, continuacion) {
  const tipos = detectarPreguntasRapidas(text).slice(0, 3);
  ...
  if (!respuestas.length) return false;

  await responderAgenda(from, respuestas.join("\n\n"));
  await responderAgenda(from, continuacion);
  return true;
}
...
async function guardarFechaYMostrarHorarios(from, fecha) {
  ...
  // Si el usuario dejó abierto el menú y respondió cuando ya cerró la jornada,
  // avanzamos al siguiente día realmente disponible.
  if (!slots.length) {
...
async function manejarDiaCita(from, text) {
  if (esComandoFlujoNormal(text)) return false;

  if (await responderPreguntasRapidas(from, text, menuDiasDisponibles())) {
    return true;
  }
...
async function manejarDiaPersonalizado(from, text, session) {
  ...
  if (await responderPreguntasRapidas(from, text, mensajePedirOtroDia())) {
    return true;
  }
...
router.use(async (req, res, next) => {
  try {
    ...
    const tramite = tramiteDirecto(session, text);
    const esPasoAgenda = ["DIA_CITA", "DIA_PERSONALIZADO"].includes(session.step);

    if (!tramite && !esPasoAgenda) return next();

    if (esPasoAgenda && esComandoFlujoNormal(text)) {
      return next();
    }
    ...
    // Limpia el indicador temporal de consultas deshabilitadas antes de enviar
    // cualquier texto de agenda, para que sendText no sustituya el mensaje.
    updateSession(from, { step: session.step });
    ...
    if (session.step === "DIA_PERSONALIZADO") {
      await manejarDiaPersonalizado(from, text, getSession(from));
      return;
    }

    return next();
  } catch (error) {
    console.error("❌ Error en calendario CRC:", error.message);
